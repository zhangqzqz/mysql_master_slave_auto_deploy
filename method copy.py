...
def create_rep_user(master_db_args, slave_db_args):
    ...
    if m_version[0] == '8':
        create_user_sql = f"create user '{username}'@'{ip}'  identified  with  mysql_native_password by 'mcbackup';"
        grant_sql = f"grant REPLICATION SLAVE ON *.* TO '{username}'@'{ip}';"
    else:
        if rds_version==[]:
            create_user_sql = f"create user '{username}'@'{ip}' identified by 'mcbackup';"
            grant_sql = f"grant REPLICATION SLAVE ON *.* TO '{username}'@'{ip}';"
        else:
            create_user_sql = f"create user '{username}'@'%' identified by 'mcbackup';"
...
def start_slave(master_db_args, slave_db_args,slave_os_args, mode):
    ...
    if 'normal' in mode:
        ...
        if change_pos_sql == 'file not access':
            ...
            slave_config_sql = f"change master to master_host='{master_db_args[0]}',master_user='{username}',master_password='mcbackup',master_port={master_db_args[2]},master_log_file='{master_log_file}',master_log_pos={master_log_pos};"
        else:
            slave_config_sql = f"change master to master_host='{master_db_args[0]}',master_user='{username}',master_password='mcbackup',master_port={master_db_args[2]},master_log_file{change_pos_sql[35:]}"
    elif 'gtid' in mode:
        slave_config_sql = f"change master to master_host='{master_db_args[0]}',master_user='{username}',master_password='mcbackup',master_port={master_db_args[2]},MASTER_AUTO_POSITION = 1;"
